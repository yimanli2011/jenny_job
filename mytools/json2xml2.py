...
_ANNOTATION_SAVE_PATH = 'D:/liyiman/datasets/jsonToXml/xmlDataSet/Annotations'
...
def createObjectNode(doc, attrs):
    ...
    if attrs['points'][0][0] > attrs['points'][1][0]:
        tempx = attrs['points'][0][0]
        attrs['points'][0][0] = attrs['points'][1][0]
        attrs['points'][1][0] = tempx
    if attrs['points'][0][1] > attrs['points'][1][1]:
        tempy = attrs['points'][0][1]
        attrs['points'][0][1] = attrs['points'][1][1]
        attrs['points'][1][1] = tempy
    createChildNode(doc, 'xmin', str(int(attrs['points'][0][0])),
                    bndbox_node)
    createChildNode(doc, 'ymin', str(int(attrs['points'][0][1])),
                    bndbox_node)
    createChildNode(doc, 'xmax', str(int(attrs['points'][1][0])),
                    bndbox_node)
    createChildNode(doc, 'ymax', str(int(attrs['points'][1][1])),
                    bndbox_node)
...
if __name__ == "__main__":
    ##json文件路径和图片路径,
    json_path = "D:/liyiman/datasets/jsonToXml/jsonDataSet/Annotations/"
    img_path = "D:/liyiman/datasets/jsonToXml/xmlDataset/JPEGImages/"
    ...
    for imageName in fileList:
        ...
        for i in json_list:
            ...
            if (saveName == imgName):
                ...
                for j in range(len(ann['shapes'])):
                    # print("该图片中有几个框:", len(ann["shapes"]))
                    object_node = createObjectNode(doc, ann["shapes"][j])
